...
(async function () {
  ...
  function validateStep(step) {
    ...
    if (step === 1) {
      ...
      const wordCount = elements.wordCount.value.trim();

      if (!title) {
        elements.pageTitle.classList.add("field-error");
        isValid = false;
      ...
      if (!url) {
        elements.pageUrl.classList.add("field-error");
        isValid = false;
        if (!errorMessage) errorMessage = "Page URL is required.";
      } else if (!isValidUrlPath(url)) {
        elements.pageUrl.classList.add("field-error");
        isValid = false;
        errorMessage = "Please enter a valid URL path.";
      } else {
      ...
      if (!language) {
        ...
        isValid = false;
        if (!errorMessage) errorMessage = "Language is required.";
      } else if (
        state.languages &&
        ...
        elements.addPageLanguage.classList.add("field-error");
        isValid = false;
        errorMessage = "Please select a language from the defined list.";
      } else {
        elements.addPageLanguage.classList.remove("field-error");
      }

      if (!wordCount || isNaN(parseInt(wordCount)) || parseInt(wordCount) < 0) {
        elements.wordCount.classList.add("field-error");
        isValid = false;
        if (!errorMessage)
          errorMessage = "Word count must be a positive number.";
      } else {
        elements.wordCount.classList.remove("field-error");
      }
    } else if (step === 2) {
      const primaryKeyword = elements.primaryKeyword.value.trim();
      if (!primaryKeyword) {
        elements.primaryKeyword.classList.add("field-error");
        isValid = false;
        errorMessage = "Primary keyword is required.";
      } else {
        elements.primaryKeyword.classList.remove("field-error");
      }
    }
  ...
  const elements = {
    ...
    settingsSecondaryPercent: document.getElementById(
      "settingsSecondaryPercent",
    ),
    ...
    editRecommendationsBody: document.getElementById(
      "edit-recommendations-body",
    ),
  ...
  async function saveState() {
    ...
    saveTimeout = setTimeout(async () => {
      ...
      const { error } = await supabase
        .from("orvella")
        .upsert(
          { id: currentUser.id, data: dataToSave, updated_at: new Date() },
          { onConflict: "id" },
        );
      if (error) {
  ...
  function attachTagsAutocomplete(input) {
    ...
    function getCurrentTagFragment() {
      ...
      const prefix = value.substring(
        0,
        lastCommaIndex >= 0 ? lastCommaIndex + 1 : 0,
      );
    ...
    function updateSelection(items) {
      items.forEach((item, i) =>
        item.classList.toggle("selected", i === currentSelectedIndex),
      );
    }
    ...
    function showSuggestions() {
      ...
      const matches = state.tags.filter((tag) =>
        tag.toLowerCase().startsWith(fragment.toLowerCase()),
      );
      if (!matches.length) {
        list.classList.remove("active");
        return;
  ...
  function initEditPrioritySelector(initialValue) {
    const options =
      elements.editPrioritySelector.querySelectorAll(".priority-option");
    options.forEach((opt) => {
  ...
  async function handleEditPageSave() {
    ...
    if (langValue) {
      if (
        state.languages &&
        state.languages.length > 0 &&
        !state.languages.includes(langValue)
      ) {
    ...
    const secondaryKeywords = parseKeywords(
      elements.editSecondaryKeywords.value,
    );
  ...
  function syncDistributionInputs() {
    ...
    elements.settingsSecondaryPercent.value =
      state.anchorDistribution.secondary;
  ...
  function goToStep(step) {
    ...
    elements.formSteps.forEach((s) =>
      s.classList.toggle(
        "is-active",
        parseInt(s.getAttribute("data-step")) === step,
      ),
    );
  ...
  async function handlePageSubmit(event) {
    ...
    const secondaryKeywordsVal = parseKeywords(
    ...
    if (!isValidUrlPath(url)) {
      showToast("Please enter a valid URL path.");
      elements.pageUrl.classList.add("field-error");
      return;
    }
    ...
    const existingActual = state.pages.find(
      (p) =>
        p.url.toLowerCase() === normalizedUrl &&
        (p.pageType || "actual") === "actual",
    );
    const existingPlanned = state.pages.find(
      (p) =>
        p.url.toLowerCase() === normalizedUrl &&
  ...
  function computeRelationScore(sourcePage, targetPage) {
    const sourceTopics = new Set(
      getTopicTags(sourcePage).map((t) => t.toLowerCase()),
    );
    const targetTopics = new Set(
      getTopicTags(targetPage).map((t) => t.toLowerCase()),
    );
  ...
  function buildRecommendations() {
    ...
    const targets = pages.filter(
      (p) => p.canReceiveLinks && p.tags && p.tags.length > 0,
    );
    ...
    for (const source of sources) {
      ...
      for (const target of targets) {
        ...
        const score =
          100 *
          (0.5 * relation + 0.35 * linkNeed + 0.15 * normalizedImportance);
      ...
      for (const cand of candidates) {
        if (slots <= 0) break;
        const { target } = cand;
        if ((currentInbound[target.id] || 0) >= targetDesired[target.id])
          continue;
    ...
    newAutoRecs.forEach((rec) => {
      ...
      const deficitPrimary = getDeficit(
        primaryCount,
        newAutoRecs.length,
        state.anchorDistribution.primary,
      );
      const deficitSecondary = getDeficit(
        secondaryCount,
        newAutoRecs.length,
        state.anchorDistribution.secondary,
      );
      const deficitLsi = getDeficit(
        lsiCount,
        newAutoRecs.length,
        state.anchorDistribution.lsi,
      );
      ...
      if (targetPage.secondaryKeywords.length)
        availableTypes.push({ type: "secondary", keyword: null });
      if (targetPage.lsiKeywords.length)
        availableTypes.push({ type: "lsi", keyword: null });
  ...
  function getObservedLinks() {
    return state.recommendations.filter(
      (rec) => (rec.linkType || "observed") === "observed",
    );
  }
  ...
  const overviewCalculator = {
    computeDensityScore(pages, links) {
      ...
      const totalTarget = pagesWithOutbound.reduce(
      ...
      const pageLevelScore =
        totalWeight > 0
          ? pageScores.reduce((sum, ps) => sum + ps.score * ps.weight, 0) /
            totalWeight
          : 0;
      ...
      return {
        ...
        extra: {
          ...
          optimal: pageScores.filter((ps) => ps.score >= 80 && ps.score <= 100)
            .length,
          overlinked: pageScores.filter((ps) => ps.score > 100 && ps.weight > 0)
            .length,
    ...
    computeHealthScore(pages, links) {
      ...
      links.forEach((link) => {
        ...
        if (source && target && source.canLinkOut && target.canReceiveLinks)
          compliantLinks++;
      });
      const permissionScore = totalLinks
        ? Math.round((compliantLinks / totalLinks) * 100)
        : 100;
      const pagesWithIncoming = pages.filter((p) => p.canReceiveLinks);
      let reachabilityScore = 100;
      if (pagesWithIncoming.length > 0) {
        const totalImportance = pagesWithIncoming.reduce(
          (sum, p) => sum + (p.priority || 3),
          0,
        );
        ...
        reachabilityScore = totalImportance
          ? Math.round((reachableImportance / totalImportance) * 100)
          : 0;
      }
    ...
    computeStrategyScore(pages, links, anchorDistribution) {
      ...
      const totalBudget = pages
        .filter((p) => p.canLinkOut)
        .reduce((sum, p) => sum + Math.floor((p.wordCount || 0) / 300), 0);
      const totalImportance = targetPages.reduce(
        (sum, p) => sum + (p.priority || 3),
        0,
      );
      const expectedIncoming = {};
      targetPages.forEach((p) => {
        expectedIncoming[p.id] =
          totalBudget > 0
            ? (totalBudget * (p.priority || 3)) / totalImportance
            : 0;
      });
      const actualIncoming = {};
      targetPages.forEach((p) => {
        actualIncoming[p.id] = 0;
      });
      links.forEach((l) => {
        if (actualIncoming[l.targetId] !== undefined)
          actualIncoming[l.targetId]++;
      });
      const totalActual = links.length;
      const totalExpected = Object.values(expectedIncoming).reduce(
        (sum, val) => sum + val,
        0,
      );
      ...
      const primaryPercent = totalAnchorLinks
        ? Math.round((primaryCount / totalAnchorLinks) * 100)
        : 0;
      const secondaryPercent = totalAnchorLinks
        ? Math.round((secondaryCount / totalAnchorLinks) * 100)
        : 0;
      const lsiPercent = totalAnchorLinks
        ? Math.round((lsiCount / totalAnchorLinks) * 100)
        : 0;
      ...
      const anchorScores = targetPages.map((p) => {
        ...
        if (incoming.length === 0)
          return {
            ...
            fit: 100,
            confidence: 0,
            expected: expectedIncoming[p.id],
          };
        const total = incoming.length;
        const pCount = incoming.filter(
          (l) => l.anchorType === "primary",
        ).length;
        const sCount = incoming.filter(
          (l) => l.anchorType === "secondary",
        ).length;
        ...
        const fit = pScore * 0.35 + sScore * 0.45 + lScore * 0.2;
      ...
      let anchorFit = 0;
      const totalWeightedFit = anchorScores.reduce(
        (sum, s) => sum + s.fit * s.expected * s.confidence,
        0,
      );
      const totalWeight = anchorScores.reduce(
        (sum, s) => sum + s.expected * s.confidence,
        0,
      );
      if (totalWeight > 0) anchorFit = totalWeightedFit / totalWeight;

      const finalScore = Math.round(
        importanceAlignment * 0.7 + anchorFit * 0.3,
      );
  ...
  function renderOverviewGauges() {
    ...
    const densityCard = document.querySelector(
      '.gauge-card[data-gauge-type="density"]',
    );
    const strategyCard = document.querySelector(
      '.gauge-card[data-gauge-type="strategy"]',
    );
    ...
    const health = overviewCalculator.computeHealthScore(
      actualPages,
      observedLinks,
    );
    ...
    setGaugeLabels(
      healthCard,
      health.finalScore + "%",
      getHealthStatus(health.finalScore),
    );
    ...
    const density = overviewCalculator.computeDensityScore(
      actualPages,
      observedLinks,
    );
    ...
    const strategy = overviewCalculator.computeStrategyScore(
      actualPages,
      observedLinks,
    ...
    setGaugeLabels(
      strategyCard,
      strategy.finalScore + "%",
      getStrategyStatus(strategy.finalScore),
    );
  ...
  function setGaugeLabels(card, mainText, statusText) {
    ...
    if (statusEl) {
      ...
      if (
        ...
        statusText === "Critical" ||
        statusText === "Very Sparse" ||
        statusText === "Overlinked" ||
        statusText === "Misaligned"
      ) {
  ...
  function initPrioritySelector() {
    const options =
      elements.prioritySelector.querySelectorAll(".priority-option");
  ...
  function bindOrvellaEvents() {
    ...
    elements.saveDistributionButton.addEventListener(
      "click",
    ...
    elements.confirmDeleteBtn.addEventListener("click", async () => {
      ...
      state.recommendations = state.recommendations.filter(
        (rec) => rec.sourceId !== pageId && rec.targetId !== pageId,
      );
      await saveState();
      closeModal(elements.confirmDeleteModal);
      closeModal(elements.editPageModal);
    ...
    elements.pageTypeToggle.addEventListener("click", () => {
      const isPlanned =
        elements.pageTypeToggle.classList.contains("is-planned");
      if (isPlanned) {
    ...
    document
      .getElementById("pagesTabPlanned")
      ?.addEventListener("click", () => {
    ...
    elements.newLanguageInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        const dropdown =
          elements.newLanguageInput.parentElement.querySelector(
            ".autocomplete-list",
          );
        if (dropdown && dropdown.classList.contains("active")) {
          return;
    ...
    document.addEventListener("click", function (e) {
      const link = e.target.closest("a.page-link[data-page-id]");
      if (link) {
        e.preventDefault();
    ...
    document
      .querySelectorAll(
        ".actions-table tbody, .orphan-table tbody, .opportunity-table tbody, .top-pages-table tbody",
      )
      .forEach((tbody) => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!pill) {
      document
        .querySelectorAll(".status-pill.active")
    ...
    actualPages.forEach((page) => {
      ...
      if (!hasIncoming) {
        ...
        if (page.tags && page.tags.length) {
          ...
          if (taxLower.some((t) => ["post", "blog", "article"].includes(t)))
            type = "Post";
          else if (
            taxLower.some((t) => ["product", "service", "pricing"].includes(t))
          )
            type = "Product";
          else if (
            taxLower.some((t) =>
              ["page", "landing", "about", "contact"].includes(t),
            )
          )
            type = "Page";
          else if (
            taxLower.some((t) => ["category", "tag", "archive"].includes(t))
          )
            type = "Category";
        }
        const outboundSuggestions = observedLinks.filter(
          (rec) => rec.sourceId === page.id,
        ).length;
  ...
  function renderOrphanTable() {
    ...
    orphans.forEach((o) => {
      const importanceClass =
        o.importance >= 4 ? "high" : o.importance >= 3 ? "medium" : "low";
      html += `<tr data-page-id="${o.id}">
                <td><a href="${o.url}" class="page-link" target="_blank">${displayUrl(o.url)}</a></td>
                <td>${o.type}</td>
  ...
  function computePriorityActions() {
    ...
    actualPages.forEach((page) => {
      ...
      if (page.priority >= 4 && incoming < 2 && incoming > 0) {
        const alreadyOrphan = actions.some(
          (a) => a.page.id === page.id && a.issue === "Orphan page",
        );
        if (!alreadyOrphan) {
          actions.push({
            priority: "High",
  ...
  function computeInternalLinkOpportunities() {
    ...
    state.recommendations.forEach((rec) => {
      projectedInbound[rec.targetId] =
        (projectedInbound[rec.targetId] || 0) + 1;
    });

    const sources = actualPages.filter(
      (p) => p.canLinkOut && p.tags && p.tags.length > 0,
    );
    const targets = allPages.filter(
      (p) => p.canReceiveLinks && p.tags && p.tags.length > 0,
    );
    ...
    for (const source of sources) {
      ...
      const currentOutbound = state.recommendations.filter(
        (rec) => rec.sourceId === source.id,
      ).length;
  ...
  function renderOpportunitiesTable() {
    ...
    let html = "";
    opps.forEach((item) => {
      html += `<tr data-page-id="${item.sourcePage.id}">
                <td><a href="${item.sourcePage.url}" class="page-link" target="_blank" title="${item.sourcePage.url}">${item.sourcePage.title}</a></td>
                <td>${item.anchor}</td>
                <td><a href="${item.targetPage.url}" class="page-link" target="_blank" title="${item.targetPage.url}">${item.targetPage.title}</a></td>
                <td>${item.score}%</td>
            </tr>`;
    });
    tbody.innerHTML = html;
  ...
  function computePlannedPagesToConvert() {
    const actualPages = state.pages.filter(
      (p) => (p.pageType || "actual") === "actual",
    );

    const intentLinks = state.recommendations.filter(
      (rec) => rec.linkType === "intent",
    );

    const result = [];
    actualPages.forEach((page) => {
      const incomingIntents = intentLinks.filter(
        (rec) => rec.targetId === page.id,
      );
      if (incomingIntents.length > 0) {
        result.push({
          page: page,
          incomingCount: incomingIntents.length,
        });
      }
    });
    return result;
  ...
  function renderPlannedToConvertTable() {
    const tbody = document.getElementById("planned-convert-body");
    const badge = document.getElementById("planned-convert-badge");
    const panel = document.getElementById("planned-to-convert-panel");
    if (!tbody || !panel) return;
  ...
  async function convertPlannedToActual(pageId) {
    const page = state.pages.find((p) => p.id === pageId);
    if (!page) return;

    const intentLinks = state.recommendations.filter(
      (rec) => rec.targetId === pageId && rec.linkType === "intent",
    );

    if (intentLinks.length === 0) {
      showToast("No intent links found for this page.");
      return;
    }

    intentLinks.forEach((rec) => {
      rec.linkType = "observed";
    });
    ...
    try {
      await saveStateNow();
      showToast(
        `Converted ${intentLinks.length} intent links to observed for "${page.title}"`,
      );
      renderPlannedToConvertTable();
      renderOverviewGauges();
    } catch (err) {
  ...
  function computeAnchorDistribution() {
    ...
    observedLinks.forEach((rec) => {
      ...
      if (type === "primary") {
        primaryCount++;
        primaryKeywords[kw] = (primaryKeywords[kw] || 0) + 1;
      } else if (type === "secondary") {
        secondaryCount++;
        secondaryKeywords[kw] = (secondaryKeywords[kw] || 0) + 1;
      } else if (type === "lsi") {
        lsiCount++;
        lsiKeywords[kw] = (lsiKeywords[kw] || 0) + 1;
      } else {
        otherCount++;
        otherKeywords[kw] = (otherKeywords[kw] || 0) + 1;
      }
    });
    ...
    function sortKeywords(obj) {
      return Object.entries(obj)
        .sort((a, b) => b[1] - a[1])
        .map(([kw, count]) => ({ keyword: kw || "(no anchor)", count }));
    }

    const primaryPercent = total ? Math.round((primaryCount / total) * 100) : 0;
    const secondaryPercent = total
      ? Math.round((secondaryCount / total) * 100)
      : 0;
    const lsiPercent = total ? Math.round((lsiCount / total) * 100) : 0;
    const otherPercent = total ? Math.round((otherCount / total) * 100) : 0;
    ...
    return {
      ...
      lsiCount,
      otherCount,
      topPrimary: sortKeywords(primaryKeywords),
      topSecondary: sortKeywords(secondaryKeywords),
      topLsi: sortKeywords(lsiKeywords),
      topOther: sortKeywords(otherKeywords),
    };
  }
  ...
  function renderDistributionRow() {
    const MAX_ITEMS_PER_CATEGORY = 9;
    ...
    if (bar) {
      const totalLinks =
        ...
        dist.otherCount;
      if (totalLinks === 0) {
        bar.innerHTML = `<div style="width:100%; text-align:center; color:var(--muted); padding:4px 0; font-size:13px;">No internal links yet</div>`;
      } else {
    ...
    if (keywordsContainer) {
      ...
      keywordsContainer.querySelectorAll(".keyword-item").forEach((item) => {
        item.addEventListener("click", function () {
          ...
          if (anchorText) {
            openAnchorDetailsModal(anchorText);
          }
        });
      });
    }
  ...
  function renderPagesList(filterType = "actual") {
    ...
    filteredPages.forEach((page) => {
      ...
      const incomingCount = state.recommendations.filter(
        (rec) =>
  ...
  function openAnchorDetailsModal(anchorText) {
    ...
    const countEl = document.getElementById("anchorDetailsCount");
    const tbody = document.getElementById("anchor-details-body");
    const emptyState = document.getElementById("anchor-details-empty");

    if (!modal || !titleEl || !countEl || !tbody || !emptyState) return;

    titleEl.textContent = anchorText || "(empty anchor)";

    const observedLinks = getObservedLinks();
    const matchedLinks = observedLinks.filter(
      (rec) => (rec.anchorText || "").trim() === anchorText.trim(),
    );

    countEl.textContent = `${matchedLinks.length} links found`;
    ...
    let html = "";
    matchedLinks.forEach((rec) => {
      const source = state.pages.find((p) => p.id === rec.sourceId);
      const target = state.pages.find((p) => p.id === rec.targetId);

      const sourceName = source ? source.title : "Unknown Page";
      const sourceId = source ? source.id : null;

      const targetName = target ? target.title : "Unknown Page";
